...
<div class="row">
    <div class="col">
        <h4 class="color-primary mb-4">City Listing</h4>
    </div>
    <div class="col">
        <button data-toggle="modal" data-target="#addModal" class="btn btn-primary float-right">Add City</button>
    </div>
</div>
...
{{ Form::open(['action' => 'Admin\StateController@update','id'=>'add_data_form','method'=>'POST','autocomplete'=>'off']) }}
...
{{ Form::open(['action' => 'Admin\StateController@update','id'=>'edit_data_form','method'=>'POST','autocomplete'=>'off']) }}
            ...
            {{ Form::hidden('id','',['id'=>'edit_id','class'=>'form-control','required']) }}
					<div class="row">
						<div class="col-lg-12 col-md-12">
                            ...
                            <div class="form-group">
								{{Form::label('edit_state_id','State')}}
                                {{Form::select('state_id', $states, '',['class'=>'form-control','id'=>'edit_state_id']) }}
                            </div>
						</div>
					</div>
      </div>
...
<script>
     $(document).ready(function()
     {
         ...
         $("#edit_country_id").on("change",function(){
             $.get_state_list($("#edit_country_id"),$("#edit_state_id"));
         });
...
$(document).on('click','.editModalOpenBtn',function(e){
           e.preventDefault();
           var params = { id : $(this).attr('data-id')};
           var url    = '{{route('city.show')}}';
           function fn_success(result)
           {
                $("#edit_id").val(result.data.id);
                $("#edit_name").val(result.data.name);
                $("#edit_state_id").val(result.data.state_id);
